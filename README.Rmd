...
g<-ggplot(hexad_long, aes(x = Determinant, y = Score, fill = Determinant)) +
  # wider violin
...
kable(ordinal_cr_results, caption = " Oridinal CR for  Hexad-12 Subscales", digits = 3)
...
R_boot <- num_bootstrap   # use 10000 for final paper
...
if (!exists("fit") || !inherits(fit, "lavaan")) {
  ...
  fit <- cfa(cfa_model, data = hexad_data, std.lv = TRUE, estimator = "WLSMV")
}

lat_cor <- lavaan::lavInspect(fit, "cor.lv")

# Order factors consistently for reporting
desired_order <- c("Philanthropist","Socializer","FreeSpirit","Achiever","Player","Disruptor")
ord <- intersect(desired_order, colnames(lat_cor))
lat_cor <- lat_cor[ord, ord, drop = FALSE]
...
lat_ci <- subset(
  pe,
  op == "~~" & lhs != rhs &
    lhs %in% ord & rhs %in% ord,
  select = c(lhs, rhs, std.all, ci.lower, ci.upper)
)

# Rename columns for clarity
colnames(lat_ci) <- c("Factor 1", "Factor 2", "Latent r", "95% CI Lower", "95% CI Upper")

# Sort alphabetically
lat_ci <- lat_ci[order(lat_ci$`Factor 1`, lat_ci$`Factor 2`), ]


kable(lat_ci, caption = " Latent correlations among Hexad determinants", digits = 3)
...
kw_results <- hexad_grouped %>%
  ...
  # Extract statistics
  mutate(
    statistic = map_dbl(test_res, "statistic"),
    df        = map_dbl(test_res, "parameter"),
    p_value   = map_dbl(test_res, "p.value")
  ) %>%
...
kable(kw_formatted[,-7], caption = "Kruskal–Wallis test for Hexad determinants by gaming frequency")
```
...
# 2. Keep rows with non-missing Gender
data_gender <- merged_hexad[!is.na(merged_hexad$Gender), ]

# Check size
dim(data_gender)

# 3. Run Kruskal–Wallis for each determinant
results_list <- lapply(hexad_vars, function(var_name) {
  
  # Build temporary dataset
  df_sub <- data.frame(
    gender = data_gender$Gender,
    score  = data_gender[[var_name]]
  )
  
  # Remove missing scores
  df_sub <- df_sub[!is.na(df_sub$score), ]
  
  # Number of gender groups with data
  n_groups <- length(unique(df_sub$gender))
  
  # If fewer than 2 groups, skip test
  if (n_groups < 2 || nrow(df_sub) == 0) {
    return(data.frame(
      Determinant = var_name,
      n          = nrow(df_sub),
      n_groups   = n_groups,
  ...
  data.frame(
    ...
    n           = nrow(df_sub),
    n_groups    = n_groups,
    statistic   = round(unname(kt$statistic),2),
    df          = unname(kt$parameter),
    p_value     = round(kt$p.value, 3)
  )
})

# 4. Combine, clean, adjust p-values
kw_results <- do.call(rbind, results_list)
kw_results$p_adj_BH <- p.adjust(kw_results$p_value, method = "BH")

kw_results_tbl <- as_tibble(kw_results[, -7])


kable(kw_results_tbl, caption = "Kruskal–Wallis test for Hexad determinants by Gender")
...
age_kendall <- hexad_dets %>%
  map_df(function(det) {
    ...
    tibble(
      Determinant = det,
      n          = sum(idx),
      tau        = round(unname(kt$estimate), 3),
      p_value    = round(kt$p.value, 3)
    )
  }) %>%
  mutate(
    p_adj_BH = round(p.adjust(p_value, method = "BH"), 5)
  )
...
kw_results <- hexad_dets %>%
  map_df(function(det) {
    df_sub <- merged_hexad_age %>%
      filter(!is.na(AgeGroup), !is.na(.data[[det]]))

    kw <- kruskal.test(df_sub[[det]] ~ df_sub$AgeGroup)

    tibble(
      Determinant = det,
      n          = nrow(df_sub),
      n_groups   = n_distinct(df_sub$AgeGroup),
      statistic  = round(as.numeric(kw$statistic), 2),
      df         = as.numeric(kw$parameter),
      p_value    = round(kw$p.value, 3)
    )
  }) %>%
  mutate(
    p_adj_BH = round(p.adjust(p_value, method = "BH"), 5)
...
kable(kw_results[,-7], caption = "Kruskal–Wallis test for Hexad determinants by Age Group")
...
sig_dets    # determinants that need a Dunn post-hoc test

dunn_results <- sig_dets %>%
  map_df(function(det) {
    df_sub <- merged_hexad_age %>%
      filter(!is.na(AgeGroup), !is.na(.data[[det]]))
...
# Hexad determinants
hexad_dets <- c("philanthropist", "socializer", "freeSpirit",
                "achiever", "player", "disruptor")

kw_country_results <- hexad_dets %>%
  map_df(function(det) {
    df_sub <- hexad_top10 %>%
      filter(!is.na(Country), !is.na(.data[[det]]))

    kw <- kruskal.test(df_sub[[det]] ~ df_sub$Country)

    tibble(
      Determinant = det,
      n           = nrow(df_sub),
      n_groups    = n_distinct(df_sub$Country),
      statistic   = round(as.numeric(kw$statistic), 2),
      df          = as.numeric(kw$parameter),
      p_value     = kw$p.value
    )
...
kable(kw_country_results[,-7], caption = "Kruskal–Wallis test for Hexad determinants by Age Group")
